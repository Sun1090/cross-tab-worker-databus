...
| Load Policy | Adaptive weighting by message rate or byte count | ✅ Implemented | Opt-in `loadWeighting` ({ messageRateWeight, byteRateWeight }): workers sample their own fan-out traffic per heartbeat and publish it with the worker record; new-route owner selection adds the normalized per-second rates to the topic count. Default (unset) keeps pure topic-count routing; existing routes stay sticky. CPU-based weighting remains Planned |
