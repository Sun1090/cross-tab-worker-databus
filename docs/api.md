...
- A publication is delivered to the handlers that were registered when its delivery began. A `subscribe()` made from inside a handler does not receive the message being dispatched, and an `unsubscribe()` made from inside a handler still does: the exact-topic list and the matching wildcard patterns are both collected before the first handler runs, so a handler cannot change the fan-out of a message already in flight.
- Subscriptions are automatically queued when the transport is not yet ready, including while a transport recovery is pending: they are held behind the recovery gate and issued once the reopen succeeds instead of being written to the connection that just reported `error`. What is issued is the subscription **still wanted** at that moment: a `subscribe()` released before its queued call ran is not sent, and a release whose topic was re-subscribed in the meantime is not sent either.
