...
**不要用 `dist-tags.latest` 来判断发布是否发生。** 包文档端点与 tarball URL 是两条不同的读取路径，而文档端点会落后于产物。0.21.30 发布时实测：`curl -s https://registry.npmjs.org/<pkg>` 仍报 `latest` 是上一个版本、`versions` 里没有 `0.21.30`（`time['0.21.30']` 也不存在），与此同时 `curl -I …/-/<pkg>-0.21.30.tgz` 返回 `HTTP 200`，`npm pack <pkg>@0.21.30 --registry https://registry.npmjs.org` 也成功写出了 tarball。那一刻阻塞门禁还在自己的重试预算之内，所以上面两件事在同一分钟里同时为真。要回答"发布到底成没成"，去 HEAD 那个 tarball URL 并试一次 pack——它们摸到的是产物本身；`latest` 只回答"消费者默认拿到哪个版本"，那是另一个问题，而且会更晚才稳定。`npm view <pkg> version` 在这之上还有第三条路径：它可能来自 npm 的**本地** packument 缓存，于是在发布前查过一次的机器上，即使全局 `latest` 已经前进，它仍会返回被取代的旧版本——这正是本门禁把目标版本固定取自 `PUBLISHED_VERSION`、并拒绝与 `package.json` 不一致的推断值的原因。不要因为一次元数据读取看起来过期就重推、移动或替换 tag；发布出去的 tag 是不可变的。
