...
0.21.32 已于 2026 年 9 月 25 日发布。项目会先持续完成可靠性发布，再进入 1.0.0 稳定性冻结。
...
- **那次"发布成功但闸门失败"的排查：预算改成测量出来的和，而不是凭感觉。** `0.21.30` 的 tag 运行把 48 × 7.5 秒的已发布消费者预算整个耗在 `ETARGET` 上，而针对同一个 tag 重跑时这一步只用了 1 秒。这个上限其实是两段延迟之和，两段现在都测过：`npm publish` 返回确认早于 registry 自己给这条版本盖的 `time[<version>]`——0.21.29 是 74.8 秒，0.21.28 是 96.8 秒，0.21.27 是 248.7 秒，0.21.30 是 310.0 秒；而 `npm pack` 用来解析的 packument 是以 `cache-control: public, max-age=300` 提供的，也就是说路径上任何一份副本按设计都可以旧一整个缓存生命周期。第二段也正是前三次绿色运行那可疑一致的 305 / 304 / 304 秒在报告的东西——一个在没人解释之前只会被读成噪声的规律。于是预算改为 96 × 7.5 秒 = 720 秒，覆盖 610 秒。究竟是哪一份副本提供了过期视图，这里是故意留空的：纯 npm 本地缓存的解释会预测重跑同样要等满 300 秒，而它没有。
