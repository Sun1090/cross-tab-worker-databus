...
还有一张形状相同的 Map **没有**上限，值得在这里点名，免得这一段被读成"所有缓存都已封顶"：`wildcardPublishCache` 把具体 topic 映射到匹配它的本地通配模式，或用 `null` 表示"扫描过了，没有匹配"。本 Worker 每发布到一个新的 topic，它就多出一条记录，只有生命周期拆除（`pause()` 与 `stop()`）会清空它。实测：对 1,200 个互不相同的 topic 调用 `publish()` 之后，`knownTopics` 仍是 500 条，而这张 Map 存有 1,200 条。给它加上限不是"挑一个数字"那么简单，因为它的两种取值都参与决定运行时行为：淘汰一条模式记录，会重跑该记录本来要阻止的首次本地分发；淘汰一条 `null` 记录会重新扫描，而如果这期间有通配模式被分配下来，那一次发布就会本地分发而不是按 owner 路由。所以先需要的是一条淘汰规则，而不是一个上限值；在规则选定之前，这份增长就是这层记忆化诚实的代价。
