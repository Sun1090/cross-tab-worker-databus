...
- **失败通知顺序**：向 `onError` 上报 open 失败之前，`openTransport()` 已清除 transport-ready 标记、拆掉初始启动的 cluster、安装失败 transport 的 stop gate，并清除自身的 `startPromise`。因此 error handler 观察到的是一个已经 settle 的失败生命周期；在该回调中同步调用 `start()` 会在 stop gate 之后开启全新尝试，而不是共享刚刚 reject 的 promise。真实 open 仍在飞行时的普通并发 start 仍共享同一个 promise。
