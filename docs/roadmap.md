...
0.21.44 was released on September 26, 2026. The project is intentionally continuing through reliability-focused releases before a 1.0.0 stability freeze.
...
- **Two shipped copies of a retired claim are corrected where they are widest.** The architecture guide carried the per-`clusterKey` storage isolation claim in a *dedicated* section and as an absolute above its key listing; both now name the `replay.persistence` store as outside that boundary, since its IndexedDB object store is keyed by the plaintext Topic and its database name never sees the cluster key. The protocol section's description of the key/plaintext pairing now matches what `0.21.42` enforces: unconditional, with a missing field being a disagreement of the same kind. Both languages.

## 0.21.42 delivered scope

- **A guard written to tolerate a missing field it never had to tolerate.** `AGENTS.md` claimed both point-to-point handlers carrying the `topicKey`/`topic` pair enforce it; re-auditing the claim rather than the code showed they disagreed, because `handleControlMessage` guarded the comparison with `message.topicKey !== undefined &&` while `handleRouteReleasedMessage` never carried that clause. The tolerance had no referent to protect — the field has been required since the initial commit, so no released version ever omitted it, and the clause arrived in the same commit as the check — and its only reachable traffic was a forger, who could make `getSnapshot().assignedTopics` report a topic the tab never subscribed until the next reconcile swept it. Measured consequence: no durable ownership minted, nothing written to storage, hence patch. The general form is now written down beside the protocol: a tolerance for a missing field is a claim about which peers exist, so read the field's history in the protocol type before writing one.
- **The storage-plaintext invariant was enforced on keys by accident and on values by nothing.** Three mutations, measured: a route record carrying a `topic` field beside its `topicKey` failed 1 test in 969 (a new sweep arm, alone), while a plaintext-bearing key also tripped two pre-existing cases that are about something else. The sweep now checks keys *and* values after every step rather than only on the quiesced end state, and `tests/storage-writers.test.ts` enumerates every durable write in `src/`, so the two documented opt-ins are a checked list rather than a claim — its topic half exists because the obvious word test was measured passing a leak that travelled through a variable named `topicKey`.
...
- **The rule is now exercised across the release cycle, not at whatever moment the clone happens to sit in.** `allowedPreambleVersions()` is extracted from the live-file case and tabled over ten rows: prep, the tag build, the record commit, two releases behind, a preamble naming a version that was never tagged, a clone carrying a newer tag, and double-digit minors where a lexical sort picks the wrong winner. Before and after, measured: with only the live case, two of the rule's three legs — including the one that broke release run 159 — survived every mutant tried against them. After the table all four die, and where they die is itself the point: two land on the tag-build row, one on the double-digit-minor row, and the member that admits a released version's own tag fails **nowhere in the live files** — on a prep tree, where `package.json`'s version is not tagged yet, that leg contributes nothing an end-state read can see, so it is decidable only in a synthesized moment.
- **The lag sweep's printed median is named for what it is.** Its `p50` is the upper-middle element rather than the mean of the middle two, so adding one sample to an odd-length series moves the figure on its own: the modern gap window went `97.3 → 126.4`, where the interpolated median of those same values is `111.85`. Pinned in `tests/publication-lag.test.ts`, whose `n=2` fixture is the only shape that exposes the whole sorted list.

## 0.21.39 delivered scope

- **One release in this changelog was dated on a day nothing happened.** `## [0.20.10]` said 2026-09-02 while its tag (`2026-09-03T12:27:29+08:00`) and its registry record (`2026-09-03T04:28:11Z`) are the same instant on the same day under either reading; the release before it was tagged at 21:33 on 09-02, so the heading simply never advanced. It is corrected, and of the 151 dated sections that carry a tag it was the only one outside both readings of its own tag.
- **The new check keys on the tag, not on npm, and that choice is measured.** `tests/documentation.test.ts` now requires each section date to be the calendar day of its tag instant in either the UTC or the `+08:00` frame. Keying the same rule on `time[<version>]` would report three bad sections instead of one, because publishing finishes after midnight UTC far often enough to matter: `0.4.0` is tagged 2026-08-30T21:28Z and recorded 2026-08-31T00:03Z, and `0.20.85` has the same shape. Two of those three "findings" would have been the gate being wrong.
- **Why both frames, and what stops an empty read from passing.** Accepting only one frame fails dozens of sections that each describe a real event (measured: 84 agree in either frame, 46 only in `+08:00`, 20 only in UTC), so the rule is "some reading of the tag", not "the local day". A floor on the number of sections checked turns a clone with no tags into a failure instead of a clean pass, and an assertion that the checked rows span both tag object types catches the silent version of that — this repository tags releases both ways, and only `%(creatordate)` answers for the two. A sibling case reads the same claim from both roadmap languages, where the date is written out (`September 25, 2026` / `2026 年 9 月 25 日`) rather than as an ISO heading. The version it names must be either the release before `package.json`'s, or `package.json`'s own once that tag exists — because the preamble is advanced by the record commit, which lands *after* the tag, so on the build of a release's own tag the newest tag is exactly the version the preamble has not reached. Corrected in `0.21.40`: this sentence first read "the newest *tagged* release, not `package.json`'s", which is the rule that killed release run 159 and that the same release's fix replaced, in the test only.

## 0.21.38 delivered scope

- **A ratio described backwards in one language and correctly in the other.** The `0.21.17` block of this file said the `storage-fail` interleaving reddens the sweep "from seed 414 onward, which is four times below the sweep's depth floor". The floor is `MIN_SEEDS = 100` in `tests/coordination-invariants.test.ts`, so 414 sits more than four times *above* it — and that is the sentence's own point: the finding needs depth past what the sweep asserts as a minimum, which makes it a finder rather than a guard. The Chinese mirror had the direction right, so the two languages agreed on every digit and disagreed on the relation. No value-by-value comparison can see that; the block-paired sweep that found it compares the Nth release block of each language, then the sentences inside it.
- **Both languages now carry the same two figures for the promoted sweep tool.** The English `0.21.37` block credited the tool with "a per-call timeout" where the Chinese block said 30 秒超时. `scripts/publication-lag.mjs` sets `CALL_TIMEOUT_MS = 30_000`, so the number is now in the English sentence as well.
- **Nothing else moved.** No library code, export, option, default, frame or storage key changed; the zero-count arm ledger and the coverage aggregates are `0.21.37`'s.

## 0.21.37 delivered scope

- **A shipped instruction pointed at a directory git never had.** Both release checklists told the reader to re-derive the publication-lag series from `.gate-logs/publication-pair.mjs`, and that path is gitignored — live on the one machine that wrote it, dead in every clone and in the published tarball's own docs. Nothing can gate it: the citation sweep resolves test *titles*, and a shell command is neither a title nor a symbol. The tool is now `scripts/publication-lag.mjs`, tracked, resolving the repository and the Release workflow id through `gh` instead of hardcoding either, with a 30 s per-call timeout after the previous sweep hung 18 minutes on one `gh api`, a `reading <version>` line so a stopped run names where it stopped, and a printed line for every member its denominator drops.
- **Running it corrected three numbers in the prose that quotes it.** The sub-second consumer steps inside the measured set are two (`0.20.86`, `0.20.87`), not the six both languages carried — the other four pre-`0.20.92` releases print as `NEGATIVE` and sit outside the 45, the exact distinction the tool draws and the copied sentence lost. The `0.20.92`-onward window holds 43 releases, not the 42 `AGENTS.md` still counted, and since that number moves every release the count became the lookup. "Prints the attempt each reading came from" over-stated a flag that fires only for a non-first attempt.
- **What the series confirmed, and what the instrument itself taught.** Every per-release sample the prose quotes re-read identical — 127.2 / 76.1 / 75.5 / 75.8 / 127.3 / 126.4 s of record lag across `0.21.31`–`0.21.36`, their steps at 304–308 s, `0.21.30` alone at 310.0 s and 364.0 s, 42 of 45 steps inside 300–310 s, no gap over 310 s — so the 720 s budget and its floor test are unchanged. And the promoted script first ran *broken*: a `const` formatter scoped inside the sweep loop killed it with a `ReferenceError` after all 45 tags had been read, printing four lines and exit 1 — the formatter moved to module scope, and what made the failure diagnosable at all was the non-zero exit, since four rows from a 49-tag sweep reads like a short series rather than a broken one.
- **A copied-forward count in the progress log had never been measured.** The `bench:browser` debt read "four releases", then "five", in successive `Next` lines; the newest `bench-results/browser-*.json` is `2026-09-24T13:40:52Z`, eight minutes before `0.21.22`'s release commit regenerated `docs/benchmarks.md`, so it is fourteen (`0.21.23` on). Neither number is in a shipped file, so the correction stays in the log, where both older lines now point at the two artifacts.

## 0.21.36 delivered scope

- **Two shipped comments were contradicted by the code they describe, and both were found by counting rather than reading.** `src/centrifuge.ts`'s dispatcher doc claimed three listeners where one function is registered twice — three sources, two call sites — and the sentence ships through `dist/centrifuge.d.ts`, because a `private` member keeps its declaration doc. `src/core/trace.ts` quoted a test total that had already gone stale twice while the same sentence told the reader to re-derive it; the value is deleted, since a number written beside its own lookup is the form nobody checks. The sweep that found them counted fourteen numeric claims in `src/` against the tree and twelve held — among them `centrifuge@5.7.4`'s twelve `emit('error')` sites inside `BaseSubscription` against 23 in the whole file, where the range qualifier is exactly what makes the claim true.
- **`CONTRIBUTING.md` was read for the first time and carried four claims the repository contradicts.** Coverage thresholds stated below the blocking gate (85/80/90/85 against `vitest.config.ts`'s 98/96/98/99); a `pnpm check` gloss that named a test count and omitted lint; a release section instructing `npm version` plus `git push origin main --tags` — tag the local HEAD and push the protected branch, the two operations this repository prohibits; and `[Unreleased]` prescribed in four places while `CHANGELOG.md` carries zero such headings and non-release PRs open the versioned section directly. All four now point at the config and the checklist as the source of truth.
- **The publish-lag recipe named a parameter it never said how to choose, and the series widened from nine releases to every tag.** Reading the jobs endpoint without an attempt segment returns the *latest* attempt, so on the one re-run release the obvious short form reported −177.0 s and a 1.0 s consumer step where attempt 1 holds the real 310.0 s gap and the 364 s failure; both checklists and the workflow comment now say where to take the attempt from. Over all 48 tags from `0.20.85`: 44 non-negative ack→record gaps, maximum 310.0 s, so the 720 s propagation budget stands — and the "the registry records minutes later" premise is scoped to the `0.20.92`-onward window, because five older releases recorded within ±0.6 s, three of them before the ack returned.
- **`AGENTS.md`'s directory map is complete and gated.** It named 16 of the 43 files under `tests/`, so every harness, prose gate, release gate and bench file added after the block was written was invisible to the session that reads it first. It now covers every tracked `src`/`tests`/`e2e` file plus one line per code directory, and a new case enforces it with the section's *edges* pinned — because a fence extraction that slips one position makes "every name appears in the block" true of a document that merely discusses those files elsewhere. Five mutants, five distinct failures.
- **Two prose gates had silently wrong scopes.** The privacy gate walked `examples/` with no extension filter, so it regex-scanned 2.9 MB of gitignored bundle and sourcemap that CI never builds — a red-at-home, green-in-CI channel; its scope is now the repository's own index, pinned on fabricated input so it reads the same built or clean. `CONTRIBUTING.md` joined the citation, empty-section and relative-link sweeps, and each addition was probed on its own: two of the three fail with the array entry and pass with only that entry removed, while the citation leg has nothing in the file to fire on and is reported as preventive only.

## 0.21.35 delivered scope

- **The release gate could declare a published version unpublished, and it did.** `scripts/verify-release-version.mjs` read the packument with a bare `npm view <pkg> time --json`, so it consulted the *configured* registry. Measured while preparing this release on a machine whose `npm` points at `registry.npmmirror.com`: its copy still reported `modified: 2026-09-25T00:32:51.767Z` — the `0.21.33` publish — when the gate ran at `03:41:18Z`, 3,207 s (53 min) after npmjs recorded `0.21.34` at `02:47:50.782Z`; the mirror's `modified` moved to `03:44:48.778Z`, so it had been answering without the version for 3,418 s, and the gate exited 1 with `CHANGELOG names 1 release(s) the registry has never recorded: 0.21.34`. (An earlier draft of this sentence said “75 minutes”, which is the gap between two *previous* stamps and not an elapsed time in this story.) The header's inference ("`time[]` records never expire, so a version missing from a read that succeeded was never published") failed on its second premise: a mirror that has not *learned* of a version is not a record of its absence, and since a mirror's sync lag is not HTTP caching, `cache-control: no-cache` returns the same stale document. The read now names `https://registry.npmjs.org`, which is what makes "the registry" mean the thing whose absence is proof; re-run after the fix it prints `the 48 earlier CHANGELOG releases from 0.20.85 on are all in the registry's time map`, exit 0. `verify-published-consumer.mjs` had already pinned `--registry` on both of its reads for the same reason — this was the last npm-based registry read in the repository that did not, and both checklist languages carried the unpinned command as the re-derive instruction. Every existing case for this gate injects `RELEASE_REGISTRY_TIME`, which is exactly why none of them could see which registry is consulted; the new case runs the un-injected path against a stub `npm` on `PATH` and asserts the flag and the URL arrive as a pair.
- **A citation to a test is now checked against the suite.** `tests/documentation.test.ts` gained "resolve to a case that exists in the file they name": every `tests/<file>`'s `<name>` in `src/**`, `AGENTS.md`, `README.md` and the public docs must match exactly one `it()`/`test()` title in the file it names (18 citations across 52 files today; a trailing `…` may prefix-match, against one hit). Both halves of the scan are controlled, because both can go empty and an empty failure list looks like a clean bill either way — the title collector must find the suite's cases (977 across 40 files, floored at 500 rather than at the corpus size), the citation pattern must examine at least one, and the `e2e` corpus is asserted on its own since 937 of those 977 come from `tests/` and the total could not notice it going empty. Two scope decisions are measured: the two exempt surfaces hold 11 more citations between them (`docs/progress.md` 8, `CHANGELOG.md` 3) while the public docs hold 1 — an earlier draft of this sentence wrote "`docs/` holds 8", which had quietly counted `progress.md`, the file the gate does not scan — and exactly one of the exempt 11 does not resolve, because it quotes this gate's own failure message rather than pointing at a case; and the looser `filename … ('a name')` window was tried and rejected — across the whole scope it yields exactly one candidate, and that candidate is the `new Error(…)` message inside the very function whose doc comment names the test. Five mutants, each with its own named failure; one earlier mutant was discarded for not compiling (a dropped backslash made the regex unterminated, which reports as a suite collection error rather than as a killed behavior). The gate reddened the `AGENTS.md` bullet written about it, because the first draft quoted the two defective tokens in citation form — so the rule now says to name a token bare when the sentence is about a defect.
...
- **A load-bearing harness count had rotted in two directions at once, and one file held both values.** What `ChannelHub`'s synchronous delivery default costs is a measured number: flipping `asyncDelivery` reddens the suite. `tests/fakes.ts`, `tests/coordination-invariants.test.ts` and one `AGENTS.md` bullet said 15 post-and-assert tests and 17 failures; a different bullet in the same `AGENTS.md` said 18. Re-measured by editing the field and running the suite: **18** fail — 9 `cluster`, 6 `data-bus`, 1 `stability` (16 that post a frame and assert in the same stack) plus the hub's two own pins in `coordination-invariants`, one of which asserts the default by name. All three surfaces now carry the pair and the edit that regenerates it. **The finding is not the off-by-one — it is that one file can hold a stale count and its correction at the same time**, in two bullets written a cycle apart, with nothing to contradict either; a drifted line number one level up.
- **A correction can break the sentence it lands in, and the zh mirror can keep the figure the English already retired.** Three survivors of the sweep that replaced an unmeasured "75 minutes" with the four timestamps that produce it: `docs/zh/release-checklist.md` still said the mirror ran an hour behind after its English counterpart carried 3,418 s; both zh copies of the measurement read `3,207 s 秒`, the unit written twice, which is what a numeric insert leaves in a sentence whose unit was already spelled out; and two copies of the retraction itself were spliced into running text, producing `…not an elapsed time in this story. so the gate exited 1 with …` and `…) and the gate exited 1 …`. Nothing compiles prose, so no gate can see the last class, and no phrase-grep finds the first — a paraphrase shares no substring with the text that was fixed, which is why the closing check greps for the *figure* across languages. `AGENTS.md` gained the rule plus the scan recipe (a full stop followed by a lowercase function word over the 70 living-surface files, inline code stripped, a lone letter before the dot read as an abbreviation); it reported **one** real hit in the tree — a sentence in `docs/progress.md`, repaired earlier in this branch at `7168810`, so a run after that one finds none — and the scan is graded by the two pre-fix commits it still finds — its first pattern required whitespace straight after the stop and so cleared the bracketed form, and its first abbreviation filter matched the `e` of `evidence.` and called the tree clean over a genuine defect. The scan then caught this session splicing a second instance while writing the bullet about the first, which is the argument for the check being a scan rather than a re-read.
...
- **The `storage-fail` fuzz interleaving that found this is measured and deliberately not shipped** — it reddens the
  sweep on the pre-fix code from seed 414 onward, which is more than four times the sweep's 100-seed depth floor, so it is a finder
