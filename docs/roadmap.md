...
The DataBus lifecycle audit now also pins failed-reopen stop-gate reuse, late rejection isolation for a superseded initial open, and generation isolation for late message/status/error callbacks from a retired transport. Mutation checks prove each regression fails when its lifecycle guard is removed.
