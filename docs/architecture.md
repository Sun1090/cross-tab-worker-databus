...
| `assignedTopics` | `WorkerClusterRuntime` | `topicKey` | Plaintext `topic` | Set on receiving `CONTROL/SUBSCRIBE`; cleared on `CONTROL/UNSUBSCRIBE` or handoff, or by `reconcileAssignedTopics` once the durable route stops naming this worker — that last one is what releases an adopted flood | The authoritative "what I own" set; drives `isAssigned` and load |
| `knownTopics` | `WorkerClusterRuntime` | `topicKey` | Plaintext `topic` | FIFO scan capped at 500 that skips owned keys and the key being remembered, so an all-owned cache lets the cap slip | The reverse-lookup cache; also the only source of plaintext in storage-less mode |
...
- **Heartbeat + reconcile loop** (default `3000 ms`, `heartbeatIntervalMs`). On every tick each Worker refreshes its own record and runs a reconcile pass: prunes Workers past `workerTtlMs`, orphaned subscribers whose Tab is no longer active, and orphaned routes that have no subscribers and exceed the TTL; repairs a confirmed route that names this Worker after its assignment was cleared (see the invariant below); recomputes its own active/standby role; re-writes its subscriber records; and re-sends `CONTROL/SUBSCRIBE` for any route that still lacks `confirmedAt` — which also recovers control messages lost on the channel.
...
- **Stranded-self-route repair.** The inverse of the drift above: a durable route *confirmed* to this Worker while `assignedTopics` no longer carries the topic. Every write that settles ownership stamps the assignment in the same task as the confirmation, so the combination can only appear when the assignment is cleared afterwards — `pause()` clears the map unconditionally but rewrites only the routes it can hand off, and a handoff whose final flush is cut short by a failing storage write (the teardown then discards what is still queued) leaves the old record pointing at the departing owner. When that worker id registers again, every peer defers to a live, confirmed owner that holds nothing, and no Tab keeps a transport subscription for the topic. The repair therefore belongs to the named Worker and to nobody else: if it still has a local handler it reclaims the route through the same self-assignment `subscribe()` performs, leaving the record and its generation exactly as peers recorded them; otherwise it deletes the record and lets a remaining subscriber elect an owner it can act on, flushing the deletion *before* broadcasting `REGISTRY` so a peer that reconciles on the nudge reads the repaired state. Routes still awaiting their confirmation are left alone — taking one over would put two transport subscriptions on the same topic, the overlap the strict handoff exists to prevent (pinned by regression).
