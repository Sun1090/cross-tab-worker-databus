...
- **Failure notification ordering**: Before an open failure is reported to `onError`, `openTransport()` has cleared the transport-ready flag, torn down an initial-start cluster, installed the failed transport's stop gate, and cleared its own `startPromise`. Error handlers therefore observe a settled failed lifecycle. A synchronous `start()` retry starts a fresh attempt chained behind that stop gate instead of sharing the promise that rejected; ordinary concurrent starts during a genuinely in-flight open still share one promise.
