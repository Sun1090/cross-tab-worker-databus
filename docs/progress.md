...
- **One measurement detour worth keeping.** `pnpm bench:compare --fail-above-pct 50` failed twice here with `dedup1000Ms` at +107 % then +121 %, and "passed" in between at +6.8 %. Since the branch is provably comment-only, that could not be the cause, and the archive says why: the metric is **bimodal**, ~11–12 ms in `04-20`/`04-55-37`/`05-33-21` and ~22–27 ms in `04-55-23`/`05-30-15`/`05-30-28`, with two consecutive runs **fourteen seconds apart on identical code** reading 27.7 and 11.0. A ±50 % window around a median of a bimodal series is therefore not measuring the code — which is also why the gate reported a −56.7 % "regression" when a fast sample followed slow ones. Filed as task #61 (in scope: "Bench baselines and trend-document corrections"); a mid-investigation note for future readers: the slow reports are evidence, not pollution, so do not clean them out of `bench-results/` to make a compare go green.
- **Blockers:** none. Standing: the wildcard-pattern replay-pruning decision (#194) remains the user's; TypeScript 7 is still blocked upstream (`typescript-eslint` 8.70.1 peers `typescript >=4.8.4 <6.1.0`).
...
- **Next task:** task #61 (the bimodal browser-bench baseline) is the highest-priority open item found by this pass, since it decides whether the trend gate can be trusted at all; then the remaining half of this audit's surface — internal comments in JS-emitting modules, which ship in the bundles and were not covered by a `.d.ts`-scoped read.
- **Updated:** 2026-09-23.
