...
- **What survived, named so nobody re-digs it.** Seven distinct rows ✓; `(0,0,1,0)` at 4 ✓ (the one pinned `pendingStop` premise); the recovery-timer guard's 2 premise rows and its 47 no-early-return rows ✓, with one precision added — the callback executes **50** times and one of those is absorbed by the `timerToken` check, so "forty-nine callback executions" counted guard-reaching evaluations while reading as a total — and the companion claim (those counts contain no fuzz volume) measured rather than inferred, by running the same probe against `tests/coordination-invariants.test.ts` alone and recording no callback at all. `tests/data-bus.test.ts` still holds 198 tests ✓, `tests/property.test.ts` still builds a no-arg reporter exactly 300 times (200 + 100 loop iterations, the third loop in that file builds none) ✓, and `validation.ts`'s differential vector does name 25 values ✓.
