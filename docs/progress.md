...
- **Milestone / version:** comment-only, rides into `0.21.17` (no behavior change, so no release of its own). Branch `docs/cluster-handoff-owner-domination`, commit `e68d4aa`, rebased onto `origin/main` = `0f3d843`.
- **Status:** **done locally, PR open.**
...
- **Risk / rollback:** `git revert e68d4aa`; nothing executable moved.
