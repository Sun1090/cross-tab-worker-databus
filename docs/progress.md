...
- **Milestone / version:** test-only, plus the correction below. Branch `test/channel-token-request-without-channel` off `7f052b1`; **PR #253**.
- **Status:** **open — CI running.** Rebased onto `19ae34c` (#252), whose entry holds the Phase 157 number; the two appended at the same anchor and the conflict was resolved by hand rather than by dropping either.
