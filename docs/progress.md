...
- **Verification:** `pnpm check` 0 (typecheck, build, **913** tests, 5/5 perf gates) and `pnpm test:coverage` 0 on this branch; `pnpm test:coverage` 0 at **97.04** on `origin/main` for the comparison above; one mutation run, its one-line diff printed before the result was believed, `src/` restored by `git checkout --` afterwards. No `src/`, option, export or protocol change, so `verify:compat` / `verify:types` / `verify:pack` are untouched by construction.
