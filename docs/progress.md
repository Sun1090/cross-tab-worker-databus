...
## Phase 169 / A count that read as a description: "the identical pair" was three sites, and one had no text at all
...
- **Verification:** `pnpm typecheck` 0 and `pnpm lint` 0 on the patched tree (the change is comment text only — `git diff --stat` is one file, 18 insertions / 6 deletions, all inside `//` lines); each probe restored byte-identically by the script's `finally`, and the script prints `RESTORE: byte-identical` as its last line so a partial restore invalidates its own report.
- **Risk / rollback:** one file of comments; `git revert` removes it and no consumer-visible string changes except the comment text itself, which ships in the bundles and declarations.
- **Next:** the same symbol-grep form of audit over `data-bus.ts`'s remaining 11 arms and `replay-persistence.ts`'s 6, where the enumerations are older; and `cluster.ts`'s own remaining zero arms are now all documented, so the hunt there stays a prose task, not a coverage one.
- **Update date:** 2026-09-24.
