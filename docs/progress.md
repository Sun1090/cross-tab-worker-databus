...
- **Risk / rollback:** one early `return` in a diagnostics path; reverting `3eadd98` restores the old window. No frame, default, storage key or export moved, so the three public-surface gates see nothing. The consumer-visible difference is that a metrics window whose clock misbehaves now reports finite numbers instead of a missing average.
