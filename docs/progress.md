...
- **Next:** merge PR #248 (the unknown-frame normalization pin; its four mutants each die, aggregate branches `96.93 → 96.98`, and `cluster.ts`'s zero-arm count drops 21 → 20). Then the ledger remainder as re-derived this session, and the two CI captures now on record for the coordination sweep's single unexplained red run from Phase 148 (#245, #246, #247 and #248's `verify` jobs are four).
- **Update date:** 2026-09-24.
