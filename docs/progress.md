...
- **Milestone / version:** no release. Comment and instruction text only, on `docs/re-derive-stop-census` cut from `main` at `328dff0` (0.21.33). `git diff -U0 src/` was filtered for any added or removed line not beginning with `//` and returned nothing, so the emitted surface and the coverage ledger are untouched; `git diff --name-only` lists `src/core/data-bus.ts`, `src/core/trace.ts`, `AGENTS.md`, `docs/progress.md`.
...
- **What survived, named so nobody re-digs it.** Seven distinct rows ✓; `(0,0,1,0)` at 4 ✓ (the one pinned `pendingStop` premise); the recovery-timer guard's 2 premise rows and its 47 no-early-return rows ✓, with one precision added — the callback executes **50** times and one of those is absorbed by the `timerToken` check, so "forty-nine callback executions" counted guard-reaching evaluations while reading as a total — and the companion claim (those counts contain no fuzz volume) measured rather than inferred, by running the same probe against `tests/coordination-invariants.test.ts` alone and recording no callback at all. `tests/data-bus.test.ts` still holds 198 tests ✓, `tests/property.test.ts` still builds a no-arg reporter exactly 300 times (200 + 100 loop iterations, the third loop in that file builds none) ✓, and `validation.ts`'s differential vector does name 25 values ✓. The numeric-adjective sweep over `src/` came back clean at every site, so these are recorded rather than re-derived: `confirmRoute()` has exactly three call sites (1101, 1205, 1477 against the declaration at 1637); `connectPromise` is assigned in exactly two statements (264, 338); `metricsActive` has seven readers (286, 307, 330, 345, 383, 387, 414) and leaves `stopped` read in exactly two places (276, 409); `transportReady = false` is written at exactly the five sites its enumeration names (738, 849, 1427, 1787, 1931) with `= true` at exactly one (825); `pendingStop` is written in four places (782, 847, 1429, 1816) and assigned **non-null** in exactly two of them (847, 1816).
