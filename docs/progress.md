...
- **Change set:** `tests/fakes.ts` (the option, its doc, the deferred pump); `tests/coordination-invariants.test.ts` (the new pin, and `setAsyncDelivery(true)` on the per-seed hub); `CHANGELOG.md` (the closed question plus a new Unreleased `### Tests` block); this entry.
