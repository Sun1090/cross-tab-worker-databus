...
- **What the ordering assertion turned out NOT to pin, measured.** The test asserts the transport call order `start` (fails) → `stop` → `start` (succeeds). Replacing the whole read with `const stop = Promise.resolve()` leaves all 195 tests in the file green, and so does moving the `pendingStop` installation below the `cluster.stop()` window: `start()` chains the reopen behind `this.pendingStop` independently, so the order survives both. The first version of this branch's comment claimed the second mutation was what the test exists to fail. It is not, and the claim was replaced with the measurement.
