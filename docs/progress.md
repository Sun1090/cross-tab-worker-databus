...
- **Status:** prepared on branch `docs/claims-the-code-does-not-support`, four commits, gates run against the frozen tree; tag and publish follow merge.
...
- **Next task:** after publish, look for the next class rather than re-reading this one — the remaining unchecked prose surfaces are the README and the per-file typedoc, and `pnpm test:coverage`'s zero-count arms ledger is still open at the branch tier.
- **Updated:** 2026-09-23.
