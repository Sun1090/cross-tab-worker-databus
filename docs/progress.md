...
- **Coverage, re-derived:** `pnpm test:coverage` exits 0; `validation.ts` is now **100 / 100 / 100 / 100** with its zero-arm count 1 → 0; `coverage/coverage-final.json` gives **59 zero branch slots of 1960** across `src/` (was 60 on `main`). Aggregate branch percentage reads **96.98** here and also read **96.98** on #250 — each PR lights one arm in a different file off a `main` one arm worse than either — so the pretty table cannot resolve a single arm and the per-slot count is what this entry cites.
