...
- **Next:** (1) Open the PR, wait for verify + browser, merge, tag `v0.21.32`, publish, verify, record. (2) The standing dependency patrol — everything current except `typescript` 6.0.3 → 7.0.2, still blocked by `typescript-eslint@8.70.1`'s peer range `>=4.8.4 <6.1.0`. (3) A `bench:browser` sample on a genuinely idle host, which this release makes more overdue than usual.
- **Update date:** 2026-09-25
