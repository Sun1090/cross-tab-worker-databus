...
- **`pnpm bench:browser` remains deferred — eighth release running.** The gate window read load average **49.27 / 39.88 / 35.16** on 8 cores, so no sample was taken rather than feed the archive that `bench:compare` baselines from. `docs/benchmarks.md` still carries its 0.21.12-era sample.
