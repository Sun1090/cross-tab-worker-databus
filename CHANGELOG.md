...
### Added
- Demo chaos toggle: a "丢弃交接确认" checkbox makes the tab drop outgoing `ROUTE_RELEASED`, so a pagehide handoff can only converge through the TTL-gated stranded-handoff recovery — covered by a browser E2E asserting survivor takeover plus the `route_migration_recovery` feed row (takeover lands ~14 s, an order of magnitude past graceful-handoff timing, which itself corroborates the recovery path).
