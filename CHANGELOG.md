...
### Changed
- Dependency security: vitest upgraded 3.2 → 4.1.11 (with matching `@vitest/coverage-v8`) to clear the GHSA-82fw-gwwq-j7x9 path-traversal advisory in `@vitest/mocker`; the audit is clean again and the full suite (typecheck, 460 unit, bench, e2e) is green on the new major. One test helper's `ReturnType<typeof vi.fn>` signature needed explicit callback typing under Vitest 4's tighter mock generics.
