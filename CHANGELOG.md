...
### Changed
- `resolveTokenRequest`'s synchronous credential-throw path lost its `isCurrentBackend()` check: the provider is invoked and its `catch` reached without a single await between the backend capture and the reply, so the check could only ever read true. The asynchronous arms keep theirs. No behavior change; whole-suite branch coverage moved from 96.02% to 96.07%.
