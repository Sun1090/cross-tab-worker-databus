...
- **Open defect, named and localized.** The loop the budget above now cuts is a real behavior and is not fixed. Under a replay of seed 1046, one tab issued **2,642 transport `subscribe` calls and 2,641 `unsubscribe` calls** inside a single seed, its own `assignedTopics` entry for one topic appearing and disappearing pass after pass while the durable route for that topic never changed at all — `storage.setItem`/`removeItem` filtered on `:route:` keys recorded no competing writes across the window, and the reads inside one awaited `stop()` reached 18,379–26,719 `getItem` calls depending on the run (the sweep's median seed posts 28 channel messages; this one posts 16,763). So this is not two tabs fighting over durable ownership: it is a runtime re-electing itself for a topic it already owns, and the sweep that drops the assignment running again to make the durable record look stale. What is *not* established: which read answers stale between the re-election's `writeRoute` and the next pass. That question is where the fix starts, and the `[CHURN]` count is the number it has to drive to zero (0 seeds cut locally out of 5,000 at the shipped budget; 18 seeds exceed 500 posts and would be cut by any budget near the median).
