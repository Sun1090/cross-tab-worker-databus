...
### Fixed
- The CI `verify` gate no longer times out on the coordination fuzzer added in this range. Its 5,000 seeds cost 66–278s locally (18–76 seeds/s depending on load) and 553s+ without finishing on a shared runner, against a 120s per-test ceiling. Both seeded fuzzers now stop on a 60s wall-clock budget rather than a seed count and assert a 100-seed floor, so a loaded runner cannot pass on a handful of interleavings; the deepest mutant the coordination harness was proved against (an emptied assignment-reconcile sweep) is caught at seed 12, so the bound costs no detection power.
- The hot-path performance gates assert the fastest of three repeats instead of a single wall-clock sample. One sample measures the scheduler as much as the code — a full-suite run isolates 38 files into 38 workers, and two of the five gates failed that way while passing on their own. A quadratic routing regression still trips its ceiling (measured 13.6s against 1s on the best repeat).
