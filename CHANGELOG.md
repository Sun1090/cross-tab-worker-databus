...
- **Open defect, named and localized.** The loop the budget now cuts is a real behavior and is not fixed. Under a seed-1046 replay, one runtime rewrote its own route record for the same topic **791 times in ~110 ms**, always with the same owner and the same generation; a peer's `reconcile()` on the resulting `REGISTRY` nudge re-elected, and each side's write produced the nudge that woke the other. Instrumented counters put a single tab at 2,642 transport `subscribe` calls and 2,609 `unsubscribe` calls inside one seed, while `storage.setItem`/`removeItem` on `:route:` keys recorded **no** competing writes in the same window — so this is not two tabs fighting over durable state, it is one runtime's in-memory assignment being re-added and swept again, and the sweep is what makes the durable record look stale again. What is *not* explained: why the re-election's own `writeRoute` does not end the loop, i.e. which read answers stale between the write and the next pass. That question is the starting point of the fix, and the `[CHURN]` count is the number it has to drive to zero (0 locally out of 5,000 seeds at the shipped budget; 18 seeds exceed 500 posts).
