...
- **A caller-injected dedup clock that returns a non-finite number poisoned every comparison that reads it.** `dedup.now` is a public option, and a non-wall-clock host is exactly what it exists for — one whose reading can be *missing* rather than merely unusual. Since every comparison in the dedup manager is arithmetic on a timestamp, a single `NaN` reading did not degrade one field: the adaptive window's `elapsed >= ADAPTIVE_WINDOW_MS` never fired, the hot path's `now - timestamp > ttlMs` became `NaN > ttlMs` (false, so nothing expired), the periodic sweep pruned nothing, and `getDedupStats().ttlMs` reported **`NaN`** — which `JSON.stringify` writes as **`null`**, so a health summary or a diagnostics log carried a null where a duration belongs. Measured on the pre-fix class: `ttlMs` → `NaN`, serialized → `{"ttlMs":null}`, and a message ID stayed suppressed for as long as it stayed in the map. The manager now holds the last finite reading, which is the degradation that changes least — time stands still, expiry pauses, the adaptive window keeps its last value — where substituting `Date.now()` would silently mix two clocks and zeroing would expire the whole map on the next call. The guard sits at the clock rather than at each comparison, which is the shape this repository already uses for the same class of input in `trace.ts`, `websocket.ts` and `replay-manager.ts`. Pinned by two cases whose legs fail apart: removing the guard fails both, while removing only the constructor's fallback — the one site with no previous reading to hold — fails the second alone, because a `NaN` baseline would make every later `elapsed` non-finite too. `docs/api.md` and its Chinese mirror state the guarantee where `dedup.now` is documented.

### Added

- **The storage boundary is now stated where it was widest, not only where it was found.** `docs/configuration.md` and `docs/architecture.md` each said the coordination records are isolated per `clusterKey` and that Topic plaintext reaches a durable store only through two named opt-ins; the architecture guide carried that claim in a *dedicated* section ("Two DataBus instances with different `clusterKey` values … operate on completely isolated storage namespaces") and as an absolute above its key listing ("All keys are isolated by `createOpaqueKey(clusterKey)`"). Both now name the exception the second opt-in introduces — its IndexedDB object store is keyed by the plaintext Topic and its database name defaults to the storage prefix, so it is not partitioned by `clusterKey` at all, and `dbName` per cluster is what separates them — in both languages.
- **The key/plaintext pairing rule is now described the way the code enforces it.** The protocol table's row "`topicKey === createOpaqueKey(topic)`" became unconditionally true in `0.21.42`, but the prose beneath it still said only that a pair which *disagrees* is a substitution — which reads as a rule about mismatches and leaves a frame carrying no `topicKey` sounding like a tolerated legacy shape, which is the reading that once kept a guard in place. The shipped text now says the comparison is unconditional, that a missing field is a disagreement of the same kind, and that a custom environment should always send the field. Both languages.

## [0.21.42] - 2026-09-26

The library's central privacy claim — that a topic's plaintext never reaches localStorage — was enforced on keys by accident and on values by nothing, and the guard that protects the coordination plane's key/topic pair was written to tolerate a missing field it never had to tolerate. Both halves of that sentence were found by re-auditing claims rather than code: the first by measuring which mutations anything noticed, the second by enumerating the readers of a pair. One behavioral change, no API, option, default, frame or storage-key change.

### Fixed

- **A forged `CONTROL` frame carrying no `topicKey` skipped the check that is supposed to stop a forged one.** `handleControlMessage` guarded the pairing comparison with `message.topicKey !== undefined && …`, so a frame with the field absent passed; `handleRouteReleasedMessage` — the other reader of the same pair, and the one that *completes* a handoff — compares unconditionally. `AGENTS.md` stated that both handlers enforce the invariant, so the claim held on one reader and not the other, the same shape as `0.21.5`, when the check existed only in `handleControlMessage` and the release note described it as if it covered the channel. The tolerance had no referent to protect: `topicKey` has been a *required* field of this message variant since the initial commit, so no released version ever omitted it, and the clause arrived in the same commit that added the check rather than being inherited from a legacy peer — unlike the optional `protocolVersion` beside it, which is a real legacy tolerance. Measured before the fix, by writing the case first: the keyless frame reached the control dispatch, put the attacker's plaintext into `assignedTopics` and `knownTopics` under the key `undefined`, and made `getSnapshot().assignedTopics` report a topic the tab never subscribed to until the next reconcile swept it. No durable ownership is minted and nothing is written to storage — there is no route under that key — so the cost is a lying public snapshot for one tick, which is why this is a patch. The frame a same-origin script can post is the same unauthenticated channel as every other coordination frame, so a forged frame naming a topic with no key is a shape the receiver had to reject rather than adopt; a missing field is a disagreement, since `createOpaqueKey` returns 32 hex characters and `undefined` is none of them. Pinned by a case whose forged frame is widened at the call site (a per-field cast over a required field would typecheck, and the test suite does not type check), asserting all three surfaces the tolerance reached. Re-adding the clause fails that case and nothing else in the suite.

### Added

- **The storage-plaintext invariant is now enforced over keys *and* values, and every durable write in `src/` is enumerated.** The multi-tab coordination sweep plants a per-seed marker in each topic name and searches the whole registry for it after every step rather than only on the quiesced end state — a record the next reconcile deletes again would leave the converged state clean while the plaintext was readable in between. Measured on three mutations, the two halves were not equally covered: a route record growing a `topic` field beside its `topicKey` failed **1 test in 969**, that arm alone, while a plaintext-bearing key also tripped two pre-existing cases that are about something else (a `clusterKey` namespace-containment assertion and the pagehide leftovers assertion) — keys had incidental coverage, values had none. `tests/storage-writers.test.ts` closes the remaining gap by enumerating every durable write in `src/` and re-deriving which of them carry a topic, so the two documented opt-ins (`channelFallback: 'storage-event'`, `replay.persistence`) are a checked list rather than a claim. Its topic half is there because the obvious word test was measured *passing* a leak that travelled through a variable named `topicKey`; the term that carries the weight is `knownTopics`, the one map that turns a hash back into a name. `docs/configuration.md` and its Chinese mirror now say the two opt-ins are the complete list and that it is checked.

## [0.21.41] - 2026-09-25

The release that `0.21.39` was — a run that dies inside its own tag build, having published nothing — has no branch in the shipped release checklist, so the procedure that recovered it lived only in `AGENTS.md` and `docs/progress.md`, neither of which is in the tarball. No library behavior, export, option, default, frame or storage key changed; the coverage floors and the zero-count arm ledger are `0.21.40`'s.

### Added

- **Both `docs/release-checklist.md` languages now carry the dead-tag recovery procedure.** The section that handles a failed release run said, in three clauses, "rerun against the unchanged tag" for propagation failures, "ship a new patch version" for artifact defects, and "never move or reuse a published tag". All three are true and none applies when the run published nothing: the second leaves an unpublished version standing in `CHANGELOG.md`, which `scripts/verify-release-version.mjs` then refuses on every later tag — so one abandoned publication would block the whole series. The procedure is now written out where a maintainer will read it: the two reads that establish "nothing was published" and the way each can lie (a configured mirror answers *"not there"* about a version it has simply not learned, and a wrong repository slug answers `release not found`, which is the expected shape of the answer and therefore gets accepted); the delete-and-recreate, stated as the two ordinary pushes it is, with the immutability rule left intact because it constrains a *published* tag; the `creatordate` consequence that makes re-cutting on the same calendar day keep the release-date gate green; and the step that prevents the case altogether — create the annotated tag, re-run `pnpm check`, `pnpm lint` and the `verify:*` trio with the tag present, and only then push the ref. That last half is why `0.21.39` died where it did: the tag build is the one moment in the cycle when `package.json`'s version already carries its own tag, so a check keyed on "the newest tag" is invisible to PR CI *and* to `main`-push CI, and three green runs said nothing. Step 6 of "Before tagging" now points at that paragraph, because the reader who needs it is the one holding the tag.

### Fixed

- **`scripts/verify-release-version.mjs` offered a remedy in a form that does nothing.** Its refusal read "Each needs either its artifact published or its section marked as a version that was never released", and the second half is a prose edit — so which prose edit is the contract, and it was unstated. Measured against the exported `findUnpublishedReleases` with a four-shape fixture and a standing-section control: `## [0.20.91] - never released` is reported exactly like the plain section it is meant to excuse, while `## 0.20.91 — never released` and `## ~~[0.20.91]~~ …` come back clean. The exemption belongs to the `## [x.y.z]` heading *shape* the scan matches, never to a keyword — which is the property worth keeping (no phrase a typo can trip), and the reason the message now names the shape instead of the word. Pinned by three legs that each die to their own mutant: a keyword exemption added to the predicate fails the still-reported leg, a regex widened to accept bare headings fails the exempted leg, and reverting the message fails the leg that reads the message. The first two legs pass under a keyword exemption whether or not the third is read, which is why the wording is asserted rather than trusted to be legible. `scripts/` and `tests/` are not in `package.json` `files`, so this half reaches a maintainer in a clone and not a consumer running `npm install` — the checklist half is the shipped part.

## [0.21.40] - 2026-09-25

`0.21.39` added a gate, then replaced that gate's rule in the test -- and the prose describing the gate, written a day earlier by the release-prep PR, was in a file the fix never opened. So the notes that shipped with `0.21.39` state the requirement its own fix deleted. No library behavior, export, option, default, frame or storage key changed; the coverage floors and the zero-count arm ledger are `0.21.39`'s.

### Fixed

- **Both roadmap languages still required the preamble to name the newest \*tagged\* release.** That is the rule which killed Release run 159, and PR #328 replaced it in `tests/documentation.test.ts` with the set that holds at every buildable moment: the greatest tag strictly below `package.json`'s version, plus that version once its own tag exists. The sentence fails in exactly the state the fix was for -- a release's own tag build, where `package.json`'s tag *is* the newest tag while the preamble legitimately still names the previous release, because the record commit that advances it lands after the tag. Corrected here in `docs/roadmap.md` and `docs/zh/roadmap.md`, inside the `0.21.39` block where the claim sits. The third copy -- this file's own `0.21.39` `### Added` bullet -- stands, and the line to state is why that does not contradict `0.20.10`'s heading being corrected in the section below: that heading was never true of anything -- no event of that release happened on the day it names -- whereas this sentence was true of the release that shipped it and was superseded inside its own version's lifecycle. A wrong record is corrected; a superseded one is left standing, and the correction is filed against the copies that still instruct a reader. `0.21.35`'s three-sample sentence sits on the second footing, and note how thinly that precedent is actually honoured here: nothing in this file retracts it. The only later text that contradicts it is `0.21.37`'s series bullet, which prints `127.3` and `126.4` beside the three without naming the claim those two falsify -- the retraction itself went into `AGENTS.md` and `docs/progress.md`, neither of which is in the tarball.

### Added

- **The preamble rule's release-cycle moments are tabulated, because the live case can only ever see the one the repository happens to be in.** `allowedPreambleVersions(packageVersion, tagRefs)` is extracted from that case and exercised over ten synthetic rows: prep, tag build, record commit, two-releases-behind in each of the first two, a preamble naming an untagged version, a clone carrying a *newer* tag, and double-digit minors where a lexical sort would pick the wrong winner. That the table was needed is measured, not asserted: against the live case alone (probe and mutation record in `docs/progress.md`), deleting the "own tag counts" member reddens it, while deleting the filter that keeps tags at-or-above `packageVersion` out of the previous tag does **not**, and neither does turning `>=` into `>` -- two of the rule's three legs had no test that could see them, and one of those two *is* the run-159 bug. After the extraction all four mutants die, and the probe records where each one lands: M2 and M3 both fail the tag-build row (`>=` widened to `>`, and the filter dropped, produce the same wrong `previousTag` there), M4 -- numeric sort made lexical -- fails the double-digit-minor row with `allowed [0.9.0]`, and M1 fails only the table's record-commit row, `allowed [0.21.38]`. That last asymmetry is the finding rather than a detail: on a *prep* tree, where `package.json`'s version has no tag yet, the own-tag member contributes nothing to the live case at all, so the leg that decides whether a released version may name itself is visible only in the synthetic moment. Which is the argument for the table.
- **The lag sweep's `p50` is named and pinned.** `scripts/publication-lag.mjs` reports the upper-middle element (`sorted[floor(n/2)]`), not the mean of the two middle values; the two coincide at odd `n` and not at even `n`, so one added sample moves the printed figure with nothing changed in what is measured -- the modern gap window read `p50=97.3` at `n=45` and `p50=126.4` at `n=46`, where the interpolated median of those same values is `111.85`. `tests/publication-lag.test.ts` now pins which convention the line uses, and the header's `EXCLUDED-NON-SUCCESS` note is corrected to say it names a run the sweep passed over rather than a verdict on the tag: `v0.21.39` prints that line *and* its own reading, having shipped from a re-pointed ref.
