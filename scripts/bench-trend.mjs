...
export function buildDocs(entries) {
  ...
  const en = [
    ...
    '',
    'The comparison baseline for release gating is `pnpm bench:compare --fail-above-pct 50`, which compares the newest report against the median of the same metric in the preceding reports (50% ceiling absorbs shared-runner noise; a single previous report was a fragile baseline because the in-page hot-path metrics alternate between a fast and a slow mode on identical code). This doc records the long-run picture: values are per-metric latencies where lower is better, and the last column is the best run inside the most recent ' + BEST_WINDOW + ' reports — not an all-time record, because the in-page matrix changed measurement semantics in early September 2026 and older reports are not comparable.',
    '',
  ...
  const zh = [
    ...
    '',
    '发布门禁的对比基线是 `pnpm bench:compare --fail-above-pct 50`：将最新报告与其之前若干报告中同一指标的中位数比较（50% 上限用于吸收共享 runner 的噪声；此前只看"上一份报告"并不稳定，因为页内热路径指标在同一份代码上会在快/慢两档之间来回跳）。本文记录长期趋势：数值为逐指标延迟，越低越好；最后一列是最近 ' + BEST_WINDOW + ' 份报告内的最优值，而不是历史纪录——页内基准矩阵在 2026 年 9 月初变更过测量语义，更早的报告不可比。',
    '',
    '<!-- BENCH-TREND:BEGIN (machine-generated table) -->',
