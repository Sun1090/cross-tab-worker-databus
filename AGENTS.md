...
- A long fuzz must bound depth by **wall clock**, never by seed count alone: runner load swings the same sweep by 4x, so a count that passes locally can blow the per-test ceiling on CI. Pattern: `MAX_SEEDS` cap + a `Date.now()` budget check at the top of each iteration + `expect(completed).toBeGreaterThanOrEqual(MIN_SEEDS)` so a slow runner cannot "pass" on a handful of interleavings. Take the floor from measured mutant **kill depth**, not from how many seeds you like — the coordination harness's heaviest mutant dies at seed 12, so 100 leaves 8x the depth that detects a regression. Read `Date.now()` **after** `vi.useRealTimers()` (or before `useFakeTimers()`): vitest fakes `Date` by default, so a check inside the fake-clock region measures simulated time.
- A timing gate must sample the code, not the scheduler: vitest isolates every file into its own worker, so a single `Date.now()` sample in a full-suite run is a preemption away from failing while the same file passes alone. Assert the **fastest of N repeats** (`bestOfMs` in `tests/perf-gate.test.ts`) — a real regression is slow on every repeat. Verify the ceiling still has teeth by mutating the hot loop, and record which weaker mutants pass so the ceiling's actual sensitivity is known.
