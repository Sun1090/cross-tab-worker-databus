...
- Budget on a clock that fake timers cannot move, and verify the clock before trusting it. Vitest fakes `Date` by default and this suite advances it ~45 simulated seconds per fuzz seed, so a faked clock that outlives the file that installed it turns the budget into nonsense in both directions: the same coordination file measured 16.4s in one full run (stopped at its floor) and 60.2s alone, and CI spent 539s on it. Reading `Date.now()` "in a real-timers window" is not enough — the *baseline* can already be fake. **Correction, measured on the pinned Vitest 5 rather than assumed: `performance.now()` is not trustworthy here either** — it advances with the faked timers in the fuzz workers, so a 2.5s budget reported "out of budget" after 48ms of real time (three seeds, each pushing ~45 simulated seconds). That is masked today by the `completed >= MIN_SEEDS` conjunct on the stop check, which is also why removing it truncates the sweep at 3 seeds and fails the floor — so the conjunct cannot simply be dropped, and any wall-clock budget must first be proven against a real monotonic source (`node:perf_hooks` captured at module load, or `process.hrtime` bound before `vi.useFakeTimers()`). The sweep's own `Date`-driven cluster time stays fake on purpose.
