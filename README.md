...
- Async credential refresh bridge for the Centrifuge worker (`credentialProvider`): the Worker requests each fresh token from the main thread over a TOKEN_REQUEST/RESPONSE exchange, keeping function-valued options out of the structured-clone boundary. With `centrifuge@5.7.4` that ask is the connection `getToken`; the protocol also carries a `getChannelToken` kind, which that SDK never requests
